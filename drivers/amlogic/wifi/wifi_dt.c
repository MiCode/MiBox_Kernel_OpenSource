...
static char DEFMAC[6] = "\x10\x48\xb1\x24\x11\x81";
...
void wifi_mac_address(char *wifi_mac)
{
	memcpy(wifi_mac, DEFMAC, 6);
}
EXPORT_SYMBOL(wifi_mac_address);
...
static int __init __mac_addr_set(char *line)
{
	return mac_addr_set(line);
}
__setup("androidboot.wifimac=", __mac_addr_set);
